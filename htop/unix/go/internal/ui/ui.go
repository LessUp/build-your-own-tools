...
// KISS: only provide CPU% sorted list and exit keys
...
func (u *UI) refresh() {
	...
	u.app.QueueUpdateDraw(func() {
		if err1 == nil {
			...
			u.title.SetText("[white::b]htop-go")
		} else {
			u.title.SetText(fmt.Sprintf("[red]System metrics error: %v", err1))
...
func (u *UI) footerMessage(err error) string {
	msg := fmt.Sprintf("[yellow]q/Esc quit  •  refresh:1s  •  sort:%s (C/M/P/N)  •  Go:%s", u.sort.label(), runtime.Version())
	if err != nil {
		msg += fmt.Sprintf("  [yellow]Process data may be incomplete: %v", err)
	}
	return msg
}
